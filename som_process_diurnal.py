...
def interp_and_prep_dataset(p, levs, ds, variables=['U', 'V', 'Q', 'T']):
    time = ds.time
    lat = ds.lat
    lon = ds.lon
...
def main(do_parallel=True):
    ...
    files_to_process = sorted(glob(out_dir_remap + "*.CONUS.nc"))
    dates_to_process = [f.split('.')[-3] for f in files_to_process]
